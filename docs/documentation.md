...
<h2> Setup </h2>
<h3>1. Tool Setup</h3>
The tool is written in Go, so make sure to <a href="https://go.dev/dl/">install</a> it on your system before proceeding.
<br>
<br>

<h4>Installation Method 1: Direct Install (Recommended)</h4>
...
After installation, the `bucketloot` command will be available globally from any directory!

<h4>Verify Installation</h4>

```bash
which bucketloot
# Output: /Users/[username]/go/bin/bucketloot (or similar)

bucketloot --help
# Shows all available flags
```

<b>Note:</b> All configuration files (blacklist.txt, regexes.json, vulnFiles.json) are now embedded in the binary. You don't need these files in your working directory unless you want to reference or modify them.

<h3>2. Credentials Setup (Optional) </h3>
In order to setup the tool for the full scan mode (optional), you need to create a `credentials.json` file in your working directory (the directory from where you run bucketloot) and add the credentials for the target platforms for whom you would like to run a full scan against. At the moment, BucketLoot only supports AWS for running full mode scans, and we expect the release of another one or two modules for other platforms very soon.

Create a `credentials.json` file with the following format:
...
In order to utilise the webhook notification feature of BucketLoot, you need to create a `notifyConfig.json` file in your working directory (the directory from where you run bucketloot). This file allows you to provide the webhook URLs for Slack and Discord channels where you would like to post the notifications. [Note: It is not mandatory to provide the webhook URLs for both the platforms at once.]

Create a `notifyConfig.json` file with the following format:
...
<b>Important:</b> These configuration files (`credentials.json` and `notifyConfig.json`) should be placed in the directory from which you run the bucketloot command, not in the installation directory.
</div>
<hr style="height: 1px;">
<div id="usage">
<h2> Usage </h2>
...
<h4>1. Basic Scan </h4>
In order to run a basic scan without any extra flags, you just need to provide a target URL or target(s) file as an argument. Since bucketloot is now installed globally, you can run it from any directory:<br><br>

```bash
# Run from anywhere
bucketloot https://myvulninstance.s3.amazonaws.com/

# Or with a targets file
...
If you would like to go beyond the maximum 1000 files per bucket limit, you can run BucketLoot's Full scan mode by <a href="#setup">setting up the target platform's access credentials</a> (create credentials.json in your working directory). Currently we only support full scan mode for Amazon Web Services and expect to release the modules for other platforms very soon.
...
For this flag, it is required for you to create a `notifyConfig.json` file in your working directory. Refer to the Setup section for more details.

```bash
# Make sure notifyConfig.json is in your current directory
bucketloot https://myvulninstance.s3.amazonaws.com/ -notify
```
...
```bash
# Save with default filename (output.json)
bucketloot https://myvulninstance.s3.amazonaws.com/ -save
...
```bash
bucketloot https://myvulninstance.s3.amazonaws.com/ -search queries.txt
```

<h5>-slow</h5>

Designed for systems with low network bandwidth where the consistency of results is important, the -slow flag allows to run all the scans sequentially instead of concurrently [Fast mode] which is the default behaviour of the tool. Although this would definitely increase the overall scan time, the tool will provide consistent results while also making sure that it can run hassle-free locally.
...
```bash
bucketloot https://bucketloot-testing.blr1.digitaloceanspaces.com/
